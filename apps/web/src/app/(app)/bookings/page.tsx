...
export default function BookingsPage() {
  ...
  const [bookingToCancel, setBookingToCancel] = useState<ApiBooking | null>(
    null,
  );
  const [isCancelling, setIsCancelling] = useState(false);
  const [bookingToRefund, setBookingToRefund] = useState<ApiBooking | null>(
    null,
  );
  ...
  const {
    ...
    queryKey: queryKeys.bookings.user(
      activeTab === "refunds" ? "cancelled" : activeTab,
    ),
    queryFn: () =>
      activeTab === "refunds" || activeTab === "disputes"
        ? getUserBookings("cancelled")
        : getUserBookings(activeTab as "upcoming" | "past" | "cancelled"),
  ...
  const [bookingToDispute, setBookingToDispute] = useState<ApiBooking | null>(
    null,
  );
  const [disputeIssueType, setDisputeIssueType] =
    useState<DisputeIssueType>("COURT_UNAVAILABLE");
  const [disputeDescription, setDisputeDescription] = useState("");
  const [disputePriority, setDisputePriority] =
    useState<DisputePriority>("MEDIUM");
  ...
  const tabData: Record<"upcoming" | "past" | "cancelled", ApiBooking[]> = {
    upcoming,
    past,
    cancelled,
  };
  ...
  const hoursPlayed = visibleBookings
  ...
  function getRefundNote(booking: ApiBooking): string {
    if (booking.isRefundEligible) {
      return (
        booking.refundPolicyReason ??
        "Full refund eligible. A pending refund request will be created after cancellation."
      );
    }
    return (
      booking.refundPolicyReason ?? "This booking is not eligible for a refund."
    );
  }
  ...
  const refundMutation = useMutation({
    mutationFn: (reason: string) =>
      createRefund({ bookingId: bookingToRefund!.id, reason }),
    onSuccess: () => {
  ...
  function confirmRefund() {
    if (!bookingToRefund || refundMutation.isPending || !refundReason.trim())
      return;
        ...
        <section className="container pb-section-sm">
          ...
          <div className="space-y-3">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="h-28 w-full animate-pulse rounded-2xl border border-white/[0.06] bg-[#0C1B26]"
              />
            ))}
          </div>
        </section>
            ...
            <div className="flex flex-col justify-center p-7 md:p-9">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                <DetailChip
                  icon={CalendarDays}
                  label="Date"
                  value={formatBookingDate(nextBooking.bookingDate)}
                />
                <DetailChip
                  icon={Clock}
                  label="Time"
                ...
                <DetailChip
                  icon={MapPin}
                  label="Court"
                  value={`${nextCourt.name} · ${nextCourt.type}`}
                />
                <DetailChip
                  icon={Timer}
                  label="Price"
                  value={`Rp ${(nextBooking.finalAmount / 1000).toFixed(0)}K`}
                />
              </div>
          ...
          tabs={TABS.map((tab) => {
            const count =
              tab === "refunds"
                ...
                : tab === "disputes"
                  ? myDisputes.length
                  : tabData[tab as "upcoming" | "past" | "cancelled"].length;
            return {
              value: tab,
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="body text-[#F7F7F7]">
                          {refund.booking?.venue?.name || refund.bookingId}
                        </span>
                        <span
                        ...
                        {refund.type === "RESCHEDULE_DIFF" && (
                          <span className="caption rounded-full bg-blue-500/10 px-2 py-0.5 text-blue-400">
                            Selisih reschedule
                          </span>
                        )}
                      </div>
                      <p className="caption text-[#F7F7F7]/40 mt-2 italic">
                        &ldquo;{refund.reason}&rdquo;
                      </p>
                      <p className="caption text-[#F7F7F7]/25 mt-2">
                        Requested: {formatShortDate(refund.createdAt)}
                      </p>
                    </div>
                    <p className="price text-base text-[#F7F7F7]">
                      Rp {(Number(refund.amount) / 1000).toFixed(0)}K
                    </p>
                  </div>
                </div>
              ))
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span
                          className={`caption rounded-full px-2 py-0.5 ${DISPUTE_STATUS_STYLES[dispute.status]}`}
                        >
                          {dispute.status}
                        </span>
                        <span className="caption rounded-full bg-white/[0.04] px-2 py-0.5 text-[#F7F7F7]/40">
                          {DISPUTE_ISSUE_LABELS[dispute.issueType]}
                        </span>
                      </div>
                      <p className="caption text-[#F7F7F7]/40 mt-2 italic">
                      ...
                      <p className="caption text-[#F7F7F7]/25 mt-2">
                        Reported: {formatShortDate(dispute.createdAt)}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-md rounded-2xl border border-white/[0.08] bg-[#0C1B26] p-6 shadow-2xl">
            ...
            <div className="mt-4 rounded-xl border border-white/[0.06] bg-[#50C8C8]/5 p-4">
              <p className="body text-[#50C8C8]">
                Eligible Amount: Rp{" "}
                {(Number(bookingToRefund.refundAmount ?? 0) / 1000).toFixed(0)}K
              </p>
              <p className="body-sm mt-1 text-[#50C8C8]/70">
                {bookingToRefund.refundPolicyReason}
              </p>
            </div>
            <div className="mt-4">
              <textarea
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-md rounded-2xl border border-white/[0.08] bg-[#0C1B26] p-6 shadow-2xl">
            ...
            <div className="mt-4 space-y-4">
              <div>
                <label className="label block text-[#F7F7F7]/60 mb-1.5">
                  Issue type
                </label>
                <select
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDisputePriority(e.target.value as DisputePriority)
                  }
                  className="body w-full rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3 text-[#F7F7F7] focus:border-[#E6FA50]/30 focus:outline-none"
                  disabled={disputeMutation.isPending}
                >
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
              ...
              <button
                ...
                disabled={
                  disputeMutation.isPending || !disputeDescription.trim()
                }
                className="label btn-lime rounded-full px-5 py-2.5 disabled:opacity-40"
              >
...
function BookingRow({
  ...
  return (
    <div
      className={`flex flex-col gap-4 rounded-xl border p-4 transition-all duration-200 sm:flex-row sm:items-center ${
        muted
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <h3
            className={`heading-3 truncate ${muted ? "text-[#F7F7F7]/40" : "text-[#F7F7F7]"}`}
          >
        ...
        <p
          className={`mt-1 flex flex-wrap items-center gap-3 caption ${muted ? "text-[#F7F7F7]/25" : "text-[#F7F7F7]/40"}`}
        >
      ...
      <div className="flex flex-col gap-3 border-t border-white/[0.06] pt-3 sm:border-0 sm:pt-0 sm:flex-row sm:items-center sm:gap-2 sm:shrink-0">
        <div className="flex items-center justify-between sm:block">
          <span className="caption text-[#F7F7F7]/60 sm:hidden">
            Total Payment
          </span>
          <p
            className={`price ${muted ? "text-[#F7F7F7]/25" : "text-[#F7F7F7]"}`}
          >
        ...
        <div className="flex flex-wrap items-center gap-1.5 w-full sm:w-auto">
          ...
          {existingRefund && (
            <span
              className={`caption flex h-8 items-center justify-center rounded-lg px-2 ${
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={`caption shrink-0 rounded-full px-2 py-0.5 ${styles[status] ?? "bg-white/5 text-[#F7F7F7]/25"}`}
    >
      {status}
    </span>
  );
