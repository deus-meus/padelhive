...
export default function CourtsPage() {
  ...
  const { data: courts = [], isLoading: isCourtsLoading } = useQuery({
    ...
    queryFn: () =>
      activeVenueId ? getVenueCourtsManage(activeVenueId) : Promise.resolve([]),
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.venues.courtsManage(activeVenueId!),
      });
  ...
  const updateMutation = useMutation({
    ...
    }) => updateCourt(activeVenueId!, courtId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.venues.courtsManage(activeVenueId!),
      });
  ...
  function handleSavePricing(courtId: string) {
    ...
    if (
      Number.isNaN(weekdayOffPeak) ||
      Number.isNaN(weekdayPeak) ||
      Number.isNaN(weekendOffPeak) ||
      Number.isNaN(weekendPeak) ||
      weekdayOffPeak < 0 ||
      weekdayPeak < 0 ||
      weekendOffPeak < 0 ||
      weekendPeak < 0
    ...
    <div className="py-8">
      <section className="container">
        ...
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <h1 className="heading-1 text-[#F7F7F7]">Courts & Pricing</h1>
                ...
                >
                  ...
                  <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
                    {[...Array(4)].map((_, j) => (
                      <div
                        key={j}
                        className="h-[90px] animate-pulse rounded-xl border border-white/[0.04] bg-white/[0.02]"
                      />
                    ))}
                  </div>
                  ...
                  >
                    ...
                    <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
                      <PricingCard
                        ...
                        price={
                          isEditing
                            ? editingPrices!.weekdayOffPeak
                            : court.pricing.weekdayOffPeak
                        }
                        onChange={(val) =>
                      ...
                      <PricingCard
                        ...
                        price={
                          isEditing
                            ? editingPrices!.weekdayPeak
                            : court.pricing.weekdayPeak
                        }
                        onChange={(val) =>
                          setEditingPrices((prev) => ({
                            ...prev!,
                      ...
                      <PricingCard
                        ...
                        price={
                          isEditing
                            ? editingPrices!.weekendOffPeak
                            : court.pricing.weekendOffPeak
                        }
                        onChange={(val) =>
                          setEditingPrices((prev) => ({
                            ...prev!,
                      ...
                      <PricingCard
                        ...
                        price={
                          isEditing
                            ? editingPrices!.weekendPeak
                            : court.pricing.weekendPeak
                        }
                        onChange={(val) =>
                          setEditingPrices((prev) => ({
                            ...prev!,
...
function PricingCard({
  ...
  const displayPrice =
    typeof price === "number" ? (price / 1000).toFixed(0) : price;
  ...
  return (
    ...
    >
      ...
      {editing && <p className="caption mt-1 text-[#F7F7F7]/25">× 1,000 IDR</p>}
    </div>
  );
}
...
function AddCourtModal({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const prices = [
      ...
      weekendOffPeak,
      weekendPeak,
    ].map((p) => parseInt(p, 10) * 1000);
    if (prices.some(Number.isNaN) || prices.some((p) => p < 0)) return;
      ...
      <div className="w-full max-w-lg rounded-2xl border border-white/[0.06] bg-[#0C1B26] shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="space-y-4">
            <div>
              <label className="label block text-[#F7F7F7]/60">
                Court Name
              </label>
              <input
                type="text"
