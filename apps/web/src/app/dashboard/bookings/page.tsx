  ...
  return (
    <div className="py-8">
      <section className="container">
        {/* Header */}
        <div>
          <h1 className="heading-1 text-[#F7F7F7]">Bookings</h1>
        ...
        <div className="mt-6 space-y-3">
          {!isLoading &&
            ...
            filteredBookings.map((booking) => {
              ...
              const player =
                booking.host?.name || booking.host?.email || "Unknown Player";
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="heading-3 text-[#F7F7F7]">{player}</p>
                        <div
                          className={`flex items-center gap-1 rounded-full px-2.5 py-0.5 ${config.bg}`}
                        >
                        ...
                        {(() => {
                          ...
                          const payConfig =
                            PAYMENT_CONFIG[
                              payStatus as keyof typeof PAYMENT_CONFIG
                            ] || PAYMENT_CONFIG.pending;
                          return (
                            <span
                              className={`caption rounded-full px-2.5 py-0.5 ${payConfig.bg} ${payConfig.color}`}
                            >
                              {payConfig.label}
                            </span>
                          );
                      ...
                      <div className="caption mt-2 flex flex-wrap items-center gap-3 text-[#F7F7F7]/25">
                        ...
                        <span className="text-[#F7F7F7]/10">·</span>
                        <span>
                          {booking.startsAt?.substring(0, 5)} –{" "}
                          {booking.endsAt?.substring(0, 5)}
                        </span>
                      </div>
                    </div>
