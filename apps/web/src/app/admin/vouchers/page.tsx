...
import { Loader2, Pencil, Plus, Ticket, Trash2, X } from "lucide-react";
...
function getVoucherStatus(v: AdminVoucher): {
  label: string;
  color: string;
  bg: string;
} {
  ...
  if (v.usedCount >= v.usageLimit)
    return { label: "Exhausted", color: "text-red-400", bg: "bg-red-400/10" };
  ...
  return (
    <div className="flex flex-1 flex-col px-6 pb-6 pt-element lg:px-8 lg:pb-8">
      <div className="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <button
          onClick={openCreate}
          className="btn-lime label flex h-11 w-full shrink-0 items-center justify-center gap-2 rounded-full px-5 sm:h-10 sm:w-auto"
        >
      ...
      <div className="flex flex-1 flex-col space-y-4">
        {isLoading ? (
          [...Array(3)].map((_, i) => (
            <div
              key={i}
              className="h-28 animate-pulse rounded-2xl border border-white/[0.06] bg-[#0C1B26]"
            />
          ))
        ) : isError ? (
          <ErrorBanner
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                      <h3 className="heading-2 font-mono tracking-wide text-[#F7F7F7] break-all">
                        {v.code}
                      </h3>
                      {status && (
                        <span
                          className={`caption shrink-0 uppercase ${status.color}`}
                        >
                          {status.label}
                        </span>
                      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-md rounded-2xl border border-white/[0.08] bg-[#0C1B26] p-6 shadow-2xl">
            ...
            <p className="body-sm mt-2 text-[#F7F7F7]/40">
              This permanently removes the voucher. If it has already been used
              on bookings, the server will block deletion — deactivate it
              instead by editing and turning off &quot;Active&quot;.
            </p>
...
function VoucherFormModal({
  ...
  const [validFrom, setValidFrom] = useState(
    voucher ? voucher.validFrom.slice(0, 10) : "",
  );
  const [validUntil, setValidUntil] = useState(
    voucher ? voucher.validUntil.slice(0, 10) : "",
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/60 px-4 py-10">
      <div className="w-full max-w-lg rounded-2xl border border-white/[0.08] bg-[#0C1B26] p-6 shadow-2xl">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="text-[#F7F7F7]/40 hover:text-[#F7F7F7]"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                value={type}
                onChange={(e) =>
                  setType(e.target.value as "NOMINAL" | "PERCENTAGE")
                }
            ...
            <div>
              <label className={labelClass}>
                {type === "PERCENTAGE" ? "Value (%)" : "Value (IDR)"}
              </label>
              <input
                type="number"
                value={value}
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
          ...
          <button
            ...
            disabled={!canSubmit}
            className="btn-lime label flex items-center justify-center gap-2 rounded-full px-5 py-2.5 disabled:opacity-40"
          >
            {isSaving ? (
