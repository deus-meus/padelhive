  ...
  return (
    <div className="flex flex-1 flex-col px-6 pb-6 pt-element lg:px-8 lg:pb-8">
      ...
      {isLoading ? (
        <div className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="h-24 animate-pulse rounded-2xl border border-white/[0.06] bg-[#0C1B26]"
              />
            ))}
          </div>
        ...
        <ErrorBanner
          title="Couldn't load commission report"
          error={error}
          onRetry={() => refetch()}
        ...
        <div className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-2xl border border-white/[0.06] bg-[#0C1B26] p-5">
              ...
              <p className="price mt-2 text-[#E6FA50]">
                {formatIDR(data.totalCommission)}
              </p>
            </div>
            <div className="rounded-2xl border border-white/[0.06] bg-[#0C1B26] p-5">
              <p className="caption text-[#F7F7F7]/40">Total GMV</p>
              <p className="price mt-2 text-[#F7F7F7]">
                {formatIDR(data.totalGmv)}
              </p>
            </div>
            <div className="rounded-2xl border border-white/[0.06] bg-[#0C1B26] p-5">
                ...
                {(() => {
                  ...
                  return data.monthlySeries.map((m) => {
                    ...
                    return (
                      <div
                        key={m.month}
                        className="flex h-full w-12 flex-shrink-0 flex-col justify-end gap-2 sm:w-16"
                      >
            ...
            <div className="min-w-[800px] p-4">
              ...
              <div className="space-y-2">
                {data.venues.map((row) => (
                  <div
                    key={row.venueId}
                    className="grid grid-cols-12 items-center gap-4 rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3"
                  >
                    <div className="col-span-4 min-w-0">
                      <p className="body-sm truncate text-[#F7F7F7]">
                        {row.venueName}
                      </p>
                      <p className="truncate caption text-[#F7F7F7]/40 mt-0.5">
                        {row.city}
                      </p>
                    </div>
