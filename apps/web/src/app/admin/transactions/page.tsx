    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#F7F7F7]/20" />
          <input
            type="text"
            placeholder="Search by booking ID, user, or venue..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full rounded-xl border border-white/[0.06] bg-white/[0.02] py-2.5 pl-10 pr-4 text-sm text-[#F7F7F7] placeholder:text-[#F7F7F7]/20 focus:border-[#E6FA50]/30 focus:outline-none"
          />
        </div>
        <div className="flex items-center gap-2">
          <Filter className="h-4 w-4 text-[#F7F7F7]/30" />
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-white/[0.04]">
                <th className="px-5 py-3 caption text-[#F7F7F7]/30 font-medium">Booking ID</th>
                <th className="px-5 py-3 caption text-[#F7F7F7]/30 font-medium">User</th>
                <th className="px-5 py-3 caption text-[#F7F7F7]/30 font-medium">Venue</th>
                <th className="px-5 py-3 caption text-[#F7F7F7]/30 font-medium text-right">Amount</th>
                <th className="px-5 py-3 caption text-[#F7F7F7]/30 font-medium text-right">Commission</th>
                <th className="px-5 py-3 caption text-[#F7F7F7]/30 font-medium">Provider</th>
                <th className="px-5 py-3 caption text-[#F7F7F7]/30 font-medium">Status</th>
                <th className="px-5 py-3 caption text-[#F7F7F7]/30 font-medium">Date/Time</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((tx) => (
                <tr key={tx.id} className="border-b border-white/[0.02] last:border-0 hover:bg-white/[0.01] transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-sm text-[#F7F7F7]/70">{tx.user}</td>
                  <td className="px-5 py-3.5 text-sm text-[#F7F7F7]/70">{tx.venue}</td>
                  ...
                  <td className="px-5 py-3.5 caption text-[#F7F7F7]/30">{tx.dateTime}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="md:hidden space-y-3">
        {filtered.map((tx) => (
          <div key={tx.id} className="rounded-2xl border border-white/[0.06] bg-[#0C1B26] p-4">
            ...
            <p className="text-sm text-[#F7F7F7]/70">{tx.user}</p>
            <p className="caption text-[#F7F7F7]/30 mt-0.5">{tx.venue}</p>
            ...
            <p className="caption text-[#F7F7F7]/20 mt-2">{tx.dateTime}</p>
          </div>
        ))}
      </div>
      ...
      {filtered.length === 0 && (
        <div className="rounded-2xl border border-white/[0.06] bg-[#0C1B26] p-12 text-center mt-4">
          <p className="caption text-[#F7F7F7]/30">No transactions match your filters</p>
        </div>
      )}
    </div>
  );
