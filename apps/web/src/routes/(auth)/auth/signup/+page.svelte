<script lang="ts">
import { AlertCircle, Loader2, Lock, Mail, User } from "lucide-svelte";
...
<div class="container flex min-h-[calc(100vh-8rem)] items-center justify-center py-12">
  <Card class="w-full max-w-md p-8">
    <div class="text-center mb-8">
      <h1 class="text-2xl font-extrabold text-[#F7F7F7]">Create Account</h1>
      <p class="mt-2 text-xs text-white/60">Join Padelhive to book padel courts instantly</p>
    </div>
    ...
    <form onsubmit={handleRegister} class="space-y-4">
      <div>
        <label for="name" class="block mb-1.5 text-xs font-medium text-white/70">Full Name</label>
        <div class="relative">
          <User class="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-white/40" />
          <input
            id="name"
            type="text"
            bind:value={name}
      ...
      <div>
        <label for="password" class="block mb-1.5 text-xs font-medium text-white/70">Password</label>
        <div class="relative">
    ...
    <p class="mt-6 text-center text-xs text-white/60">
      ...
      <a href="/auth/login" class="font-semibold text-[#E6FA50] hover:underline">Sign in</a>
    </p>
  </Card>
</div>
