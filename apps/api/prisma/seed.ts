...
function getWibTime(offsetDays: number, hours: number, minutes = 0): Date {
  ...
  return new Date(
    midnight.getTime() + (hours - 7) * 60 * 60 * 1000 + minutes * 60 * 1000,
  );
}
...
async function main() {
  ...
  const bali = await prisma.venue.create({
    data: {
      ...
      imageUrl:
        "https://images.unsplash.com/photo-1622279457486-640ca4a4ea6b?auto=format&fit=crop&w=1200&q=80",
      photos: [
        "https://images.unsplash.com/photo-1622279457486-640ca4a4ea6b?auto=format&fit=crop&w=1200&q=80",
      ],
  ...
  const jakarta = await prisma.venue.create({
    data: {
      ...
      imageUrl:
        "https://images.unsplash.com/photo-1554068865-24cecd4e34d8?auto=format&fit=crop&w=1200&q=80",
      photos: [
        "https://images.unsplash.com/photo-1554068865-24cecd4e34d8?auto=format&fit=crop&w=1200&q=80",
      ],
  ...
  const surabaya = await prisma.venue.create({
    data: {
      ...
      imageUrl:
        "https://images.unsplash.com/photo-1530915534664-4ac6423816b7?auto=format&fit=crop&w=1200&q=80",
      photos: [
        "https://images.unsplash.com/photo-1530915534664-4ac6423816b7?auto=format&fit=crop&w=1200&q=80",
      ],
  ...
  const courtUtama = await prisma.court.create({
    data: {
      venueId: surabaya.id,
      name: "Court Utama",
      type: CourtType.INDOOR,
  ...
  const courtLatihan = await prisma.court.create({
    data: {
      venueId: surabaya.id,
      name: "Court Latihan",
      type: CourtType.OUTDOOR,
  ...
  const v1 = await prisma.voucher.create({
    data: {
      ...
      usedCount: 1,
      isActive: true,
      validFrom: getWibMidnight(-30),
      validUntil: getWibMidnight(30),
    },
  });
  ...
  const bUpcoming = await prisma.booking.create({
    data: {
      ...
      courtId: courtA.id,
      voucherId: v1.id,
      bookingDate: getWibMidnight(5),
      startsAt: getWibTime(5, 9, 0),
      endsAt: getWibTime(5, 10, 0),
      durationMinutes: 60,
      status: BookingStatus.CONFIRMED,
      courtAmount: 300000,
      platformFee: 15000,
      voucherDiscount: 63000,
      finalAmount: 252000,
    },
  });
  ...
  const bCompleted = await prisma.booking.create({
    data: {
      ...
      courtId: court1.id,
      voucherId: v2.id,
      bookingDate: getWibMidnight(-14),
      startsAt: getWibTime(-14, 18, 0),
      endsAt: getWibTime(-14, 19, 30),
      durationMinutes: 90,
  ...
  const bCancelled = await prisma.booking.create({
    data: {
      ...
      venueId: surabaya.id,
      courtId: courtUtama.id,
      voucherId: v3.id,
      bookingDate: getWibMidnight(-20),
      startsAt: getWibTime(-20, 10, 0),
      endsAt: getWibTime(-20, 11, 0),
      durationMinutes: 60,
      status: BookingStatus.CANCELLED,
      courtAmount: 250000,
      platformFee: 12500,
      voucherDiscount: 26250,
      finalAmount: 236250,
  ...
  await prisma.payment.create({
    data: {
      id: "payment-upcoming",
      bookingId: bUpcoming.id,
      status: PaymentStatus.PAID,
      provider: "midtrans",
      method: "ewallet",
      amount: 252000,
      paidAt: getWibTime(-1, 9, 0),
      providerReference: "MT-UPCOMING",
    },
  });
  const pCompleted = await prisma.payment.create({
    data: {
      ...
      method: "va",
      amount: 475000,
      paidAt: getWibTime(-14, 17, 30),
      providerReference: "MT-COMPLETED",
    },
  });
  const pCancelled = await prisma.payment.create({
    data: {
      ...
      method: "ewallet",
      amount: 236250,
      paidAt: getWibTime(-21, 9, 0),
      providerReference: "MT-CANCELLED",
    },
  });
  ...
  const iAndi = await prisma.invite.create({
    data: {
      ...
      isHost: true,
      userId: usersRecord["andi@example.com"].id,
      email: "andi@example.com",
      name: "Andi Pratama",
      token: "invite-andi",
    },
  ...
  const iSari = await prisma.invite.create({
    data: {
      ...
      isHost: false,
      userId: usersRecord["sari@example.com"].id,
      email: "sari@example.com",
      name: "Sari Dewi",
      token: "invite-sari",
    },
  ...
  await prisma.invite.create({
    data: {
      ...
      isHost: false,
      userId: usersRecord["budi.player@padelhive.com"].id,
      email: "budi.player@padelhive.com",
      name: "Budi Rahmat",
      token: "invite-budi",
    },
  ...
  const rProcessed = await prisma.refund.create({
    data: {
      id: "refund-processed",
      bookingId: bCompleted.id,
      paymentId: pCompleted.id,
      amount: 50000,
      reason: "Partial refund due to court maintenance",
      status: RefundStatus.PROCESSED,
      adminNotes: "Processed via manual transfer",
      processedAt: getWibTime(-13, 10, 0),
    },
  });

  await prisma.refundEvent.create({
    data: {
      refundId: rPending.id,
      fromStatus: null,
      toStatus: RefundStatus.PENDING,
      actorUserId: usersRecord["budi.player@padelhive.com"].id,
      createdAt: getWibTime(-21, 10, 0),
    },
  });

  await prisma.refundEvent.create({
    data: {
      refundId: rProcessed.id,
      fromStatus: null,
      toStatus: RefundStatus.PENDING,
      actorUserId: usersRecord["sari@example.com"].id,
      createdAt: getWibTime(-13, 9, 0),
    },
  });
  await prisma.refundEvent.create({
    data: {
      ...
      actorUserId: usersRecord["admin@padelhive.com"].id,
      createdAt: getWibTime(-13, 10, 0),
    },
  });
  await prisma.refundEvent.create({
    data: {
  ...
  const reviewsData = [
    {
      ...
      name: "Andi Pratama",
      rating: 5,
      comment: "Fasilitas outdoor terbaik di Bali! View-nya mantap.",
      offset: -10,
    },
    {
      venueId: bali.id,
      ...
      name: "Sari Dewi",
      rating: 4,
      comment: "Asik buat main sore, tapi siang panas banget.",
      offset: -8,
    },
    {
      venueId: bali.id,
      ...
      name: "Budi Rahmat",
      rating: 5,
      comment: "Kondisi lapangan sangat terawat. Top!",
      offset: -3,
    },

    {
      ...
      name: "Andi Pratama",
      rating: 4,
      comment: "Lokasi strategis di SCBD. Parkiran agak susah kalau malam.",
      offset: -7,
    },
    {
      venueId: jakarta.id,
      ...
      rating: 5,
      comment:
        "Lapangan sangat terawat dan fasilitas showernya bersih sekali! Akan rutin main di sini.",
      offset: -14,
    }, // Replaces the old single review
    {
      venueId: jakarta.id,
      courtId: court2.id,
      userId: usersRecord["budi.player@padelhive.com"].id,
      ...
      rating: 5,
      comment:
        "AC dingin, lapangan indoor estetik. Cocok buat mabar pulang ngantor.",
      offset: -2,
    },

    {
      ...
      rating: 5,
      comment:
        "Akhirnya ada padel di Surabaya. Lapangannya standar internasional.",
      offset: -12,
    },
    {
      venueId: surabaya.id,
      courtId: courtLatihan.id,
      userId: usersRecord["sari@example.com"].id,
      name: "Sari Dewi",
      rating: 4,
      comment: "Peralatan rentalnya lengkap dan bagus-bagus kondisinya.",
      offset: -6,
    },
    {
      venueId: surabaya.id,
      ...
      name: "Budi Rahmat",
      rating: 5,
      comment: "Coach nya sangat profesional dan ramah untuk pemula.",
      offset: -1,
    },
  ];
  ...
  for (let i = 0; i < reviewsData.length; i++) {
    ...
    await prisma.payment.create({
      data: {
        ...
        method: "qris",
        amount: 315000,
        paidAt: getWibTime(data.offset, 13, 30),
        providerReference: `MT-REV-${i}`,
      },
    });
