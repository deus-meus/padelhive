...
export class AdminService {
  ...
  async getBookings(query: AdminBookingsQueryInput, user: RequestUser) {
    const {
      status,
      venueId,
      fromDate,
    ...
    return {
      items: items.map((item) => ({
        ...item,
        bookingDate: utcToWibDateStr(item.bookingDate),
      })),
      page: pageNum,
      pageSize: sizeNum,
      total,
  ...
  async getCommission(query: CommissionQueryInput) {
    ...
    const [venueGroups, venues] = await Promise.all([
    ...
    for (const row of seriesRows) {
      ...
      let bucket = seriesMap.get(month);
      if (!bucket) {
        bucket = { gmv: 0, commission: 0, bookings: 0 };
        seriesMap.set(month, bucket);
      }
  ...
  async getMetrics() {
    const todayWib = utcToWibDateStr(new Date());
    const [yyStr, mmStr] = todayWib.slice(0, 7).split("-");
    ...
    for (const b of bookings) {
      const wibDate = utcToWibDateStr(b.bookingDate);
      const mStr = wibDate.slice(0, 7);
