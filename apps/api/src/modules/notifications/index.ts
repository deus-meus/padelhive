...
export const notificationsModule = new Elysia({ prefix: "/notifications", name: "notificationsModule" })
  .use(authPlugin)
  .get("/stream", ({ user }) => {
    ...
    detail: { summary: "Subscribe to real-time notification stream (SSE)", tags: ["Notifications"] },
  })
  .get("/", ({ user }) => {
    const authed = ensureAuth(user);
    return notificationsService.findMyNotifications(authed.id);
  }, {
    detail: { summary: "Get current user notifications", tags: ["Notifications"] },
  })
  .get("/unread-count", ({ user }) => {
    const authed = ensureAuth(user);
    return notificationsService.getUnreadCount(authed.id);
  }, {
    detail: { summary: "Get unread notifications count", tags: ["Notifications"] },
  })
  .patch("/read-all", ({ user }) => {
    const authed = ensureAuth(user);
