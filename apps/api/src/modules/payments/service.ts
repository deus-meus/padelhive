...
export class PaymentsService {
  ...
  async createIntentForUser(userId: string, body: CreatePaymentIntentInput) {
    ...
    if (pendingPayment) {
      return this.stripHostUserId(pendingPayment as SelectedPayment);
    }
    ...
    return this.stripHostUserId(finalPayment as SelectedPayment);
  }

  async findPaymentForUser(id: string, userId: string) {
    ...
    return this.stripHostUserId(payment as SelectedPayment);
  }

  async markPaidForUser(id: string, userId: string) {
    const payment = await this.prisma.payment.findFirst({
      where: { id },
      select: paymentSelect,
    });
    ...
    if ((payment as SelectedPayment).booking.hostUserId !== userId) {
  ...
  async handleMidtransWebhook(payload: MidtransWebhookInput): Promise<void> {
  ...
  private stripHostUserId(payment: SelectedPayment) {
    return {
      ...payment,
      booking: {
