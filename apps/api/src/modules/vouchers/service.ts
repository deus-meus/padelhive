import { Prisma, type Voucher, VoucherType } from "@prisma/client";
import {
  BadRequestException,
  ConflictException,
  NotFoundException,
...
import type { CreateVoucherInput, UpdateVoucherInput } from "./model";

export class VouchersService {
  constructor(private readonly prisma: PrismaService = defaultPrisma) {}

  private toResponse(voucher: Voucher) {
    return {
      id: voucher.id,
      code: voucher.code,
      type: voucher.type,
  ...
  private validateVoucherFields(dto: {
    ...
    if (
      dto.type !== VoucherType.NOMINAL &&
      dto.type !== VoucherType.PERCENTAGE
    ) {
    ...
    if (
      ...
      (!Number.isInteger(dto.minPurchase) || dto.minPurchase < 0)
    ) {
      throw new BadRequestException(
        "Min purchase must be a non-negative integer",
      );
    }
    if (
      dto.maxDiscount != null &&
      (!Number.isInteger(dto.maxDiscount) || dto.maxDiscount < 0)
    ) {
      throw new BadRequestException(
        "Max discount must be a non-negative integer",
      );
    }
  ...
  async updateVoucher(id: string, dto: UpdateVoucherInput) {
    ...
    const minPurchase =
      dto.minPurchase !== undefined ? dto.minPurchase : existing.minPurchase;
    const maxDiscount =
  ...
  async deleteVoucher(id: string): Promise<{ id: string }> {
    ...
    try {
      ...
      if (
        ...
        throw new BadRequestException(
          "This voucher is referenced by bookings and cannot be deleted. Deactivate it instead.",
        );
      }
      throw e;
    }
  ...
  async findActiveVouchers(now = new Date()) {
    const vouchers = await this.prisma.voucher.findMany({
      where: {
        isActive: true,
        validFrom: { lte: now },
  ...
  async priceVoucher(
    ...
    if (voucher.minPurchase != null && amount < voucher.minPurchase) {
      throw new BadRequestException(
        "Minimum purchase not met for this voucher",
      );
    }
