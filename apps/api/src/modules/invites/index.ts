...
export const invitesModule = new Elysia({ name: "invitesModule" })
  .use(authPlugin)
  .post("/bookings/:bookingId/invites", ({ params, body, user }) => {
    const authed = ensureAuth(user);
    return invitesService.createInviteForBooking(authed.id, params.bookingId, body);
  }, {
    body: CreateInviteSchema,
    detail: { summary: "Create player invite for booking", tags: ["Invites"] },
  })
  .get("/bookings/:bookingId/invites", ({ params, user }) => {
    const authed = ensureAuth(user);
  ...
  .get("/invites/:token", ({ params }) => {
    return invitesService.getInviteByToken(params.token);
  }, {
    detail: { summary: "Get public invite details by token", tags: ["Invites"] },
  })
  .patch("/invites/:token/rsvp", ({ params, body }) => {
