...
describe("Booking creation API", () => {
  ...
  it("rejects invalid dates and non-whole-hour times", async () => {
    const service = new BookingsService(createPrisma() as never, {} as never, { createNotification: jest.fn() } as never, {} as never);
  ...
  it("rejects cancel for missing or non-owned bookings", async () => {
    ...
    }) as never, {} as never, { createNotification: jest.fn() } as never, {} as never);
